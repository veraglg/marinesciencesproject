---
title: "Are 'bioplastics' a solution to plastic pollution in the ocean?"
subtitle: |
  *Laboratory testing of Ecovio® compostable plastic potential
  biodegradation in lake and marine benthic environments*
author:
  name: "Vera Gómez-Limón Gallardo"
  orcid: 0009-0005-4889-3412
  affiliation:
    - "Universidad de las Palmas de Gran Canaria"
    - "JAE Intro ICU 2024 Scholarship at IAS-CSIC"
date-format: long
license: "CC BY-NC-SA 4.0"
number-sections: true
slide-number: true
slide-level: 2
format:
  revealjs:
    footer: "Marine Sciences Bachelor Thesis"
    transition: slide
    logo: images/logo-ulpgc-simple.png
    embed-resources: true
    preview-links: auto
theme: default
include-in-header:
  - text: |
      <style>
      #title-slide .title {
        font-size: 2em;
      }
      </style>
bibliography: references.bib
---

```{r}
#| label: setup
#| include: false
source("scripts/common.R")
```


#### About the author {.unnumbered}

- First began Marine Sciences at ULPGC in 2020
- Granted JAE intro ICU 2024 Scholarship at IAS-CSIC

#### About the study {.unnumbered}

- Supervisors:
  - PhD Blanca Landa del Castillo (IAS-CSIC)
  - PhD José González Pajuelo (i-UNAT-ULPGC)
- Licensed under [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/)
- Fully open source on [GitHub](https://github.com/vgoliga/bachelor-thesis)

<!-- #### Roadmap {.unnumbered} -->

<!-- - June 2024: Background research -->
<!-- - July 2024: Field sampling and preliminary tests -->
<!-- - August 2024: Main tests -->
<!-- - Autumn 2024: Data processing and modelling -->
<!-- - Spring 2025: Thesis writing -->

# Introduction
## Plastic pollution
### How big is the problem? {.unnumbered}

::: {.fragment}
- Ocean plastic flux: **11,000,000 Mt/y** [@reddy2020]
- Accelerating trend driven by **'business as usual'** [@oecd2022]
:::

### Where is the limit? {.unnumbered .fragment}

::: {.fragment}
- Defined **9 planetary boundaries** [@rockstrom2009]
- **6/9** boundaries transgressed [@richardson2023]
- Production **outpaces safety assessment** [@persson2022]
:::

::: footer
1 Introduction
:::

---

![](images/planetary-boundaries.png){.absolute top=0 left=0 width="650"}
![](images/beached-plastic.jpeg){.absolute top=0 right=0 width="400"}

::: {.absolute bottom=0 right=0 width="400"}
[@richardson2023]
:::

::: footer
1 Introduction
:::

::: {.notes}
How much can Earth system **tolerate** before shifting into a **less habitable state?**
:::

---

### Why is plastic so threatening? {.unnumbered}

- Plastic **is persistent** and **accumulates** in the ocean.
  - Plastic bags captured on camera at **10,000 m deep** [@jamieson2022]
  - High abundance of microplastics in **Arctic deep-sea sediment** [@bergmann2017]
- Engineered remediation is unlikely to succeed [@macleod2021]

::: footer
1 Introduction
:::

---

### What are the consequences to marine life? {.unnumbered}

:::: {.columns}
::: {.column width="60%"}
- 17% of entangled *spp.* are IUCN **Red listed** [@gall2015]
- Seabirds (*Procellariiformes*) are **highly vulnerable** to poisoning [@savoca2016]
- **Invasive species** dispersion on top of plastic debris [@barnes2005]
:::

::: {.column width="40%"}
![](images/albatross.jpeg)
:::
::::

::: footer
1 Introduction
:::

## 'Bioplastics'
### What are they made of? {.unnumbered}

:::: {.columns}
::: {.column width="60%"}
![](images/bioplastics-diagram.jpeg)
:::

::: {.column width="40%"}
Compostable does not mean environmentally biodegradable, e.g. [@farah2016]: PLA needs **60 °C** to effectively degrade
:::
::::

::: footer
1 Introduction
:::

---

### Are they environmentally safe? {.unnumbered}

::: incremental
- Depends on polymer composition and environment --- salinity, T, microbial diversity [@lott2021]
- Many **do not degrade** at sea, e.g. PLA [@royer2023]
- **Lack of consensus** around biodegradation test methods [@lavagnolo2024]
- Environmental research falls behind leaving a **knowledge gap** [@ribba2022]
:::

::: footer
1 Introduction
:::

## Objectives of the study {.center}

::: incremental
- Evaluate **Ecovio®** compostable plastic biodegradability
- Propose a **fast and environmentally relevant** test method for benthic environments [@tosin2012]
- Predicting ultimate biodegradability fitting **non-linear models** to BOD curves
:::

::: footer
1 Introduction
:::

# Materials and Methods

## Measuring principle
### How does biodegradation happen? {.unnumbered}

::: {.fragment}
$$
\require{mhchem}
\ce{Plastic + O2(g) ->[Decomposers] Biomass + CO2 ^ + Minerals}
$$
:::

### How to quatify O~2~ consumption? {.unnumbered .fragment}

::: {.fragment}
$$\ce{CO2(aq) + 2Na^+ + 2OH^- + H2O->\\ Na2CO3v + 2H2O}$$
:::

---

Biodegradation becomes a **pressure measurement**
$$PV = nRT$$

### OxiTop® Control System {.unnumbered .fragment}

![](images/oxitop.jpg){.absolute .fragment bottom=50 left=125 width="800"}

::: footer
2 Materials and Methods
:::

---




::: footer
2 Materials and Methods
:::

## Testing materials

![](images/ecovio-bag.jpg){.absolute top=100 left=0 width="600"}

::: footer
2 Materials and Methods
:::

## Field sampling

```{r}
#| echo: false
library(leaflet)

samples <- data.frame(
  lat = c(37.8320, 36.5479),
  lng = c(-5.0369, -4.6152), 
  popup = c("La Breña Reservoir", "Los Boliches Beach")
)

leaflet(samples) |> 
  addProviderTiles("Esri.WorldImagery") |> 
  addMarkers(~lng, ~lat, popup = ~popup) |> 
  setView(lat = 37, lng = -5, zoom = 6) |>
  addMiniMap() |> 
  addScaleBar()
```

::: footer
2 Materials and Methods
:::

<!-- ![](images/methods.jpg) -->

# Results

## Visual inspection {background-color="#43464B"}
### Did materials show signs of biodegradation? {.unnumbered}

::: {layout-ncol=2}

![Lake](images/visual-lake.jpg){width="500"}

![Marine](images/visual-sea.jpg){width="500"}

:::

::: footer
3 Results
:::

## BOD test results

::: panel-tabset

### Graph

```{r}
data |> filter(time %% 5 == 0) |>
  ggplot(aes(x = time, y = bod, color = test)) +
  geom_point(aes(shape = test), stat = "summary", fun = median) +
  geom_ribbon(aes(fill = test), show.legend = F, alpha = .3, linetype = 0,
              stat = "summary", fun.min = min, fun.max = max) +
  geom_line(aes(linetype = test), stat = "summary", fun = median) +
  facet_wrap(~ medium, scales = "free_y",
             labeller = as_labeller(labels_medium)) +
  scale_color_discrete(name = NULL, labels = labels_test) +
  scale_linetype_manual(name = NULL, labels = labels_test,
                        values = c("dotted", "solid", "dashed")) +
  scale_shape_manual(name = NULL, labels = labels_test, values = c(0, 16, 1)) +
  labs(x = "Time (days)", y = "BOD (mg/L)") +
  theme(legend.position = "inside", legend.position.inside = c(.15, .8))
```

### Table

```{r}
data |> filter(time == 30) |> 
  group_by(medium, test) |> 
  summarize(median = median(bod), range = str_range(bod), 
            .groups = "drop") |> 
  left_join(t_tests, by = join_by(medium, test)) |> 
  mutate(
    median = round(median, 2),
    p.value = round(p.value, 5),
  ) |> 
  mutate(
    medium = fct_recode(medium, "Lake" = "lake", "Marine" = "sea"),
    test = fct_recode(test,
                           "Cellulose (+C)" = "paper",
                           "Ecovio®" = "plastic",
                           "Control (-C)" = "control")
  ) |>
  gt() |>
  cols_label(
    medium = "Env.", test = "Test",
    median = "Median (mg/L)", range = "[Min, Max] (mg/L)", p.value = "T-test (p-value)"
  ) |> 
  cols_align("left", test) |> cols_align("center", range) |> 
  sub_missing()
```


:::

::: footer
3 Results
:::

## Non-linear modelling

```{r}
fits |> unnest(data) |> 
  ggplot(aes(x = time, y = bod)) +
  geom_point(alpha = .1) +
  geom_line(
    data = fits |> unnest(preds),
    aes(y = .fitted, color = model, linetype = model),
    linewidth = 1
  ) +
  facet_wrap(~ reactor, scales = "free_y") +
  labs(x = "Time (days)", y = "BOD (mg/L)") +
  theme(legend.position = "inside", legend.position.inside = c(.9, .05),
        legend.title = element_blank())
```

::: footer
3 Results
:::

# Discussion

::: footer
4 Discussion
:::

# Conclusion

# References {.unnumbered}
::: {#refs}
:::